  ...
  return (
    <div style={{ minHeight: "100vh", background: "#060810", color: "#fff" }}>
      ...
      <main className="checkin-private-content nova-arcade-main" style={{ minHeight: "100vh", padding: "140px 32px 80px", maxWidth: 1100, margin: "0 auto" }}>
        <section className="arcade-section"><div className="arcade-section-title"><div><span className="nova-eyebrow">{isPt ? "JOGOS NOVA" : "NOVA GAMES"}</span><h2>{isPt ? "Escolhe a tua experiência" : "Choose your experience"}</h2></div><span className="arcade-level"><Trophy size={16} /> {isPt ? "Sem competição" : "No competition"}</span></div><p className="game-muted">{isPt ? "Experiências recreativas. O desempenho não é uma avaliação médica, neurológica, cognitiva ou de concussão." : "Recreational experiences. Performance is not a medical, neurological, cognitive or concussion assessment."}</p><div className="game-grid">{arcadeGames.map((game) => <button type="button" className="game-card" key={game.id} onClick={() => launchArcadeGame(game.id)} style={{ "--game-accent": game.accent } as React.CSSProperties}><div className="game-card-art"><span>{game.icon}</span><i /></div><div className="game-card-copy"><span className="game-card-meta">{game.meta}</span><h3>{game.title}</h3><p>{game.text}</p><span className="game-play"><Play size={14} fill="currentColor" /> {isPt ? "Jogar" : "Play"}</span></div></button>)}</div></section>
