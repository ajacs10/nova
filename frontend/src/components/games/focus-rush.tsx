...
export function FocusRush({ isPt, onComplete }: GameProps) {
  const [initialRound] = React.useState(() => {
    const target = randomTarget();
    return { target, items: makeItems(0, target) };
  });
  const [round, setRound] = React.useState(1);
  const [score, setScore] = React.useState(0);
  const [combo, setCombo] = React.useState(0);
  const [bestCombo, setBestCombo] = React.useState(0);
  const [target, setTarget] = React.useState(initialRound.target);
  const [items, setItems] = React.useState<number[]>(initialRound.items);
...
function randomTarget(previous?: number) {
  const choices = SHAPES.map((_, index) => index).filter((index) => index !== previous);
  return choices[Math.floor(Math.random() * choices.length)];
}
function makeItems(round: number, target: number) {
  const distractors = Array.from({ length: 12 + round * 4 }, () => {
    const choices = SHAPES.map((_, index) => index).filter((index) => index !== target);
    return choices[Math.floor(Math.random() * choices.length)];
  });
  const targetCount = round > 5 ? 2 : 1;
  for (let index = 0; index < targetCount; index += 1) {
    distractors.splice(Math.floor(Math.random() * (distractors.length + 1)), 0, target);
  }
  return distractors;
}
