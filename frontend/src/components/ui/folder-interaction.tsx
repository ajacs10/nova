...
function FolderInteraction({ forceOpen }: { forceOpen?: boolean }) {
  const [isOpen, setIsOpen] = useState(false);
  const isFolderOpen = forceOpen ?? isOpen;
  ...
  return (
    <div className="w-full flex justify-center items-center cursor-pointer select-none">
      <div
        onClick={() => setIsOpen(!isFolderOpen)}
        className="w-full max-w-80 h-40 sm:h-52 relative wrapper"
      >
        <div
        ...
        >
          {[
            ...
            <motion.div
              ...
              animate={isFolderOpen ? page.open : page.initial}
              transition={page.transition}
              className={`absolute top-2 w-32 h-fit rounded-xl ${page.className}`}
            >
              <Page title={i === 0 ? "Tarefas" : i === 1 ? "Projetos" : "Notas"} />
            </motion.div>
          ))}
        </div>

        <motion.div
          animate={{ rotateX: isFolderOpen ? -40 : 0 }}
          transition={{ type: "spring", duration: 0.5, bounce: 0.2 }}
          className="absolute -left-[1px] -right-[1px] -bottom-[1px] z-20 h-36 sm:h-44 rounded-3xl origin-bottom flex justify-center items-center overflow-visible"
        >
          <svg
...
const Page = ({ title }: { title?: string }) => (
  <div className="w-full h-full bg-gradient-to-b from-[#E8E7F0] to-[#DCDAE8] rounded-xl shadow-lg p-3 sm:p-4 text-slate-800">
    <div className="flex flex-col gap-1.5 sm:gap-2">
      <div className="text-[10px] font-bold text-slate-700 uppercase tracking-wider">{title || "Pasta"}</div>
